 ...
 */
final class WC_Gateway_Paykka_Encrypted_Card_Support extends AbstractPaymentMethodType
{
     ...
     * @var string
     */
    protected $name = 'paykka-encrypted-card'; // 替换为你的支付网关 ID
    ...
    {
        $script_path = '/assets/js/blocks-encrypted-card.js';
        $script_url = plugin_url_paykka() . $script_path;

        wp_register_script(
            'wc-paykka-encrypted-card-gateway-blocks', // 脚本句柄
            $script_url, // 脚本路径
            array('wc-blocks-registry', 'wc-settings', 'wp-element', 'wp-i18n'), // 依赖
            '1.0.0', // 版本号
            true // 是否在页脚加载
        );
        return array('wc-paykka-encrypted-card-gateway-blocks');
    }
    /**
     * 获取支付方法的数据
     *
